...
if __name__ == "__main__":
    ...
    moviesPopularities = inputMoviesRaw.map(lambda line : (line[0], (line[1], float(line[7])))) #(movie_id, (title, popularity))
    
    inputRatings = sc.textFile(sys.argv[3]).map(lambda line : (line.split(',')[1], (line.split(',')[0], float(line.split(',')[2])))) #(movie_id, (user_id, rating))
    ...
    maxUserRatingPerGenre = ratingsWithGenresPopularities.aggregateByKey(("", 0, 0), findMaxRating, findMaxRating) #((genre, user), (title, max_rating, popularity))
    minUserRatingPerGenre = ratingsWithGenresPopularities.aggregateByKey(("", 10, 0), findMinRating, findMinRating) #((genre, user), (title, min_rating, popularity))
    
    genresUsers = inputMovieGenres.join(inputRatings).map(lambda line : ((line[1][0], line[1][1][0]), line[1][1][1])) #((genre, user), rating_of_1_movie)
    userRatingsPerGenre = genresUsers.groupByKey().map(lambda line : (line[0], len(line[1]))) #((genre, user), total_ratings)
